...
export default defineEventHandler(async (event) => {
  ...
  if (query.code) {
    ...
    // The state is single-use even when validation fails.
    deleteCookie(event, OAUTH_STATE_COOKIE, { path: '/auth/google' })

    if (!verifyOAuthState(query.state, cookieState)) {
    ...
    setCookie(event, OAUTH_STATE_COOKIE, state, STATE_COOKIE_OPTIONS)
  }

  // nuxt-auth-utils keeps config at handler creation time. Build the handler inside
  // this request so authorizationParams receives this request's nonce.
