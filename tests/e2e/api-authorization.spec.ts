...
async function expectNoLeak(
  ...
  // `task` / `tasks` 是保養提醒那兩種形狀（#122）：清單一支、勾選與取消各回一個任務。
  for (const field of ['tank', 'tanks', 'creature', 'creatures', 'waterLogs', 'series', 'task', 'tasks', 'name']) {
...
test.describe('未登入', () => {
  ...
  test('十五支 API 一律回 401', async ({ browser }) => {
    const context = await browser.newContext()
    ...
    const responses = await Promise.all([
      ...
      // 生物照片（issue #154）。上傳那一支特別要緊：它收的是 multipart，
      // 折成 400 或先把檔案讀進來的話，沒有 cookie 的人也能讓 server 收下一份檔案
      context.request.post('/api/creatures/any-creature-id/photo', {
        multipart: { file: { name: 'photo.png', mimeType: 'image/png', buffer: Buffer.from([0x89, 0x50, 0x4E, 0x47]) } },
      }),
      context.request.delete('/api/creatures/any-creature-id/photo'),
    ])

    expect(responses.map(response => response.status())).toEqual(Array.from({ length: 15 }, () => 401))
